...
${sectionParagraphsHtml(s)}
  ...
  const breadcrumbHtml = `        <nav aria-label="Breadcrumb" class="mb-10 font-mono text-[10px] uppercase tracking-[0.18em]" style="color: rgba(168, 155, 138, 0.85)">
          <ol class="flex flex-wrap gap-x-3 gap-y-1">
            ...
            <li aria-current="page" style="color: rgba(13, 11, 10, 0.7)">${esc(page.h1).toUpperCase()}</li>
          </ol>
        </nav>`;
  ...
  const faqHtml = page.faqs
    .map(
      (f) => `          <div class="mb-8">
            <h3 style="font-family: 'Fraunces', Georgia, serif; font-style: italic; font-size: 18px; line-height: 1.3; margin: 0 0 0.5rem; color: #0d0b0a; letter-spacing: 0; text-transform: none;">${esc(f.q)}</h3>
            <p>${esc(f.a)}</p>
          </div>`
    )
    .join("\n\n");
  ...
  const relatedHtml =
    related.length > 0
      ? `          <nav aria-label="Related guides" class="mt-14 border-t border-stone/30 pt-10">
            <p class="eyebrow mb-4" style="color: rgba(168, 155, 138, 0.85);">RELATED</p>
            <ul class="space-y-2">
...
<html lang="en" class="bg-paper">
  ...
  <body class="bg-paper text-ink antialiased">
    <header class="mx-auto max-w-editorial px-6 pt-8 flex items-center justify-between">
      <a href="/" class="font-display text-[22px] tracking-tight hover:opacity-70 transition-opacity" aria-label="Style.ai">
        Style<span class="opacity-70">.ai</span>
      </a>
      <a href="/guides/" class="eyebrow hover:text-ink transition-colors">← GUIDES</a>
    </header>

    <main id="main" class="mx-auto max-w-editorial px-6 pb-24 pt-12 sm:pb-32 sm:pt-16" role="main">
      <article class="guide-prose">
${breadcrumbHtml}

        <header>
          <h1>${esc(page.h1)}</h1>
          <p class="mb-6"><time datetime="${GUIDE_DATE}">Updated ${GUIDE_DATE}</time> · From Style.ai</p>
          <p>${esc(page.lead)}</p>
        </header>

${sectionsHtml}

        <section aria-labelledby="faq-${page.slug}" class="mt-16 border-t border-stone/30 pt-10">
...
${relatedHtml}
        ...
        <p class="mt-16 border-t border-stone/30 pt-10 text-center">
          <a class="cta-pill cta-pill--ink" href="/#your-turn">Pull your first look →</a>
        </p>
      </article>
    </main>

    <footer class="bg-ink py-10">
      <div class="mx-auto flex max-w-editorial flex-col items-center justify-between gap-4 px-6 sm:flex-row">
        <p class="eyebrow eyebrow--paper">STYLE.AI · 2026</p>
        <nav aria-label="Footer">
          <ul class="eyebrow eyebrow--paper flex items-center gap-6" style="color: rgba(245, 241, 234, 0.5)">
            <li><a href="/" class="transition-opacity hover:opacity-100">HOME</a></li>
            <li><a href="/privacy" class="transition-opacity hover:opacity-100">PRIVACY</a></li>
            <li><a href="/terms" class="transition-opacity hover:opacity-100">TERMS</a></li>
          </ul>
        </nav>
      </div>
    </footer>
...
function renderIndex() {
  const items = PAGES.map((p) => {
    const blurb =
      p.description.length > 140 ? `${esc(p.description.slice(0, 138))}…` : esc(p.description);
    return `          <li>
            <a href="/guides/${p.slug}" class="group block border-t border-stone/30 pt-5 pb-3 transition-colors hover:border-ink">
              <h2 style="font-family: 'Fraunces', Georgia, serif; font-style: italic; font-size: clamp(22px, 3vw, 26px); line-height: 1.25; margin: 0; color: #0d0b0a; text-transform: none; letter-spacing: -0.005em;">${esc(p.h1)}</h2>
              <p class="mt-2" style="font-family: system-ui, sans-serif; font-size: 15px; line-height: 1.55; color: rgba(13, 11, 10, 0.62); margin: 0;">${blurb}</p>
            </a>
          </li>`;
  }).join("\n");
...
<html lang="en" class="bg-paper">
  <head>
    ...
    <meta
      ...
      content="What to wear today, AI stylist and outfit-planner ideas, skin-tone-friendly colors, and wardrobe tips from your real closet."
    />
    <meta property="og:image" content="${ORIGIN}/og-cover.jpg" />
    <meta name="twitter:card" content="summary_large_image" />
    <meta
  ...
  <body class="bg-paper text-ink antialiased">
    <header class="mx-auto max-w-editorial px-6 pt-8 flex items-center justify-between">
      ...
      <a href="/" class="eyebrow hover:text-ink transition-colors">← HOME</a>
    </header>
    <main id="main" class="mx-auto max-w-editorial px-6 pb-24 pt-12 sm:pb-32 sm:pt-16" role="main">
      <article class="guide-prose">
        <p class="eyebrow mb-6" style="color: rgba(168, 155, 138, 0.85);">ISSUE №01 · FIELD NOTES</p>
        <h1>Outfit &amp; style guides.</h1>
        <p>
          Practical reads on what to wear, outfit planning from your real closet, dress codes, travel packing, and colors that flatter your skin — plus India metro seasons and occasions. Each guide links back to the app when you want to try it.
        </p>
        <ul class="mt-12 space-y-6 list-none !ml-0">
${items}
        </ul>
        <p class="mt-16 border-t border-stone/30 pt-10 text-center">
          <a class="cta-pill cta-pill--ink" href="/#your-turn">Pull your first look →</a>
